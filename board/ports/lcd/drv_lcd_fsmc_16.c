...
#define LCD_BASE        ((uint32_t)(0x60040000 | 0x0007FFFE))
#define LCD             ((LCD_TypeDef *) LCD_BASE)
*/

// #define  LCD_ADDR_BASE       0x60040000 // LCD->LCD_BASE
// #define  LCD_ADDR_DATA       0x0007FFFE // LCD->LCD_RAM //(LCD_ADDR_BASE + (1 << (LCD_REGSELECT_BIT + 2)) - 2) // ((uint32_t)(0x60040000 | 0x0007FFFE))
...
typedef struct
{
  volatile uint32_t ISR[2];  /*!< DMA interrupt status register,      Address offset: 0x00 */
  volatile uint32_t IFCR[2]; /*!< DMA interrupt flag clear register,  Address offset: 0x08 */
} DMA_TypeDef_Array;
...
#else
#define DelayMs(t)            rt_thread_mdelay(t)
...
INIT_DEVICE_EXPORT(rt_hw_fsmc_init);
